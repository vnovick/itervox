...
> ```
>
> The migrator writes a `WORKFLOW.md.bak` next to your workflow, extracts each profile's inline `prompt:` block into `.itervox/agents/<name>/INSTRUCTIONS.md`, generates a compact `.itervox/agents/<name>/SOUL.md` identity file, and patches the root `.gitignore` so both `.itervox/agents/**` and `.itervox/handoff/**` can be committed. Review the migrated `WORKFLOW.md` and the new agent files, then delete `WORKFLOW.md.bak`. See [Agent Profiles guide](https://itervox.dev/guides/agent-profiles/) for the file-backed profile reference.
>

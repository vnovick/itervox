...
:::caution
Native debate run groups are not production-ready in v0.2.0. The workflow below is an experimental pattern built from existing automations, labels, comment sentinels, and comment-only planner profiles.
:::
...
agent:
  profiles:
    planner-claude:
      ...
      prompt: |
        You are the Claude planning profile.
        Do not modify code.
        Post exactly one tracker comment starting with:
        <!-- itervox:debate:claude-plan -->
    ...
    planner-codex:
      ...
      allowed_actions: [comment]
      prompt: |
        You are the Codex planning profile.
        Do not modify code.
        Post exactly one tracker comment starting with:
        <!-- itervox:debate:codex-plan -->
        Include the recommended approach, risks, tests, and rollout plan.
    ...
    debate-moderator:
      ...
      prompt: |
        ...
        If both are present, compare the plans and post one decision comment starting with:
        <!-- itervox:debate:decision -->
```
...
| Both planners post at once | Latest-comment polling can miss one trigger | Native run-group join barrier |
...
Native debate should be modeled as an automation run group:
...
Production run groups should store participant artifacts, status, deadlines, failure policy, and moderator output separately from tracker comments. Tracker comments can mirror the final decision, but they should not be the only source of truth.

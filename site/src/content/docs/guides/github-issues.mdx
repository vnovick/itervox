...
If GitHub does rate-limit Itervox, it honours `Retry-After` for secondary limits and `X-RateLimit-Reset` once the hourly budget is spent. Queued comments and label changes wait for the reset and show a **rate limited until HH:MM** chip in the dashboard's Outbox panel — they are never dropped and never posted twice.

...
| `-logs-dir` | string | `~/.itervox/logs/<kind>/<slug>-<project>` (or `~/.itervox/logs/workflow/<project>` when no `project_slug` is set) | Directory for rotating log files |
...
| `ITERVOX_API_TOKEN` | `""` | Enables bearer-token authentication on all `/api/v1/*` routes (except `/api/v1/health`). Requests must include `Authorization: Bearer <token>`. When unset, Itervox auto-generates an ephemeral token at startup on **every** bind — including loopback — unless `server.allow_unauthenticated: true` (renamed from `server.allow_unauthenticated_lan`, which still parses as a deprecated alias). The auto-generated token, and the dashboard URL that carries it, are printed to stderr once. Pin a stable value in `.itervox/.env` for bookmarks that survive restarts. |
...
**Location:** `~/.itervox/logs/<kind>/<slug>-<project>/itervox.log` (or `~/.itervox/logs/workflow/<project>/itervox.log` when no `project_slug` is set)
